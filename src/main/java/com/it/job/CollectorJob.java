    ...
    public void execute(JobExecutionContext context)
        ...
        if (prevFileSystems != null) {
            for (int i = 0; i < fileSystems.size(); i++) {
                FileSystem fileSystemDiff = fileSystems.get(i).diff(
                        prevFileSystems.get(i));
        ...
        if (prevFileSystems != null && prevNetwork != null) {
            ResourceMetrics resource = new ResourceMetrics(cpuMetrics,
                    memoryMetrics, fileSystemsDiff, networkDiff);
